...
class Downsample(nn.Module):
    ...
    def forward(self, x, is_bn=True):  # is_bn用于确定是否使用bn层，默认为True
        x = self.conv_relu(x)+self.conv_relu1(x)
        if is_bn:
            x = self.bn(x)
        return x
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.down6 = Downsample(1024, 2048)  # 1024,8,8 -- 1024,4,4
        self.down7 = Downsample(2048, 2048)  # 1024,4,4 -- 1024,2,2

        self.up1 = Upsample(2048, 2048)  # 1024,2,2 -- 2048,4,4
        self.up2 = Upsample(4096, 1024)  # 1024,4,4 -- 1024,8,8
        self.up3 = Upsample(2048, 512)  # 2048,8,8 -- 512,16,16
        self.up4 = Upsample(1024, 256)  # 512,16,16 -- 128,32,32
        self.up5 = Upsample(512, 128)  # 512,32,32 -- 64,64,64
        self.up6 = Upsample(256, 64)  # 512,64,64 -- 64,128,128
    ...
    def forward(self, x):
        ...
        x7 = self.up1(x7, is_drop=True)#2048--2048
        x7 = torch.cat([x7, x6], dim=1)#2048+2048=4096

        x7 = self.up2(x7, is_drop=True)#4096--1024
        x7 = torch.cat([x7, x5], dim=1)#1024+1024=2048

        x7 = self.up3(x7, is_drop=True)#2048--512
        x7 = torch.cat([x7, x4], dim=1)#512+512=1024

        x7 = self.up4(x7)#1024--256
        x7 = torch.cat([x7, x3], dim=1)#256+256=512

        x7 = self.up5(x7)#512--128
        x7 = torch.cat([x7, x2], dim=1)#128+128=256

        x7 = self.up6(x7)#256--64
        x7 = torch.cat([x7, x1], dim=1)#64+64=128

        x7 = torch.tanh(self.last(x7))
